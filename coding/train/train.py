...
def load_model(run_args: RunArgs, trans_args: TransparencyArgs):
    ...
    model = AutoModel.from_pretrained(
        run_args.model_path, trust_remote_code=True, torch_dtype=torch.bfloat16
    )
...
def apply_lora(model, lora_args: LoraArgs, no_adapter: bool = False):
    ...
    cfg = LoraConfig(**vars(lora_args), task_type=TaskType.CAUSAL_LM)
    ...
    print(
        "LoRA param checksum:",
        sum(p.sum().item() for n, p in model.named_parameters() if "lora_" in n),
    )
...
# Training setup
def train_model(
    run_args, training_args, dataset_args, tokenizer, model, train_dataset, eval_dataset
):
    """
...
def main():
    ...
    run_args, dataset_args = parser_run_and_dataset.parse_json_file(
        os.path.abspath(sys.argv[2])
    )

    # Parse common training arguments
    parser_training = HfArgumentParser(
        (LoraArgs, CustomTrainingArguments, TransparencyArgs)
    )
    lora_args, training_args, trans_args = parser_training.parse_json_file(
    ...
    training_args.output_dir = os.path.join(
        training_args.output_dir, run_args.job_name, timestamp, notes
    )
    ...
    train_dataset, eval_dataset = load_data(
        dataset_args, tok, seed=training_args.data_seed
    )
    ...
    train_model(
        run_args, training_args, dataset_args, tok, model, train_dataset, eval_dataset
    )
