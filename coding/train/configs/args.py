...
class LoraArgs:
    ...
    target_modules: List[str] = (
        field(default_factory=lambda: ["q_proj", "k_proj", "v_proj", "o_proj"]),
    )  # , "gate_proj", "up_proj", "down_proj"
    modules_to_save: List[str] = (
        field(default_factory=lambda: ["transparency"]),
    )  # current implementation does not save this since we just use the transparency_config.json parameters during inference
...
class CustomTrainingArguments(TrainingArguments):
    loss_calc: str = (
        "model_weighted",
    )  # Options: "diffullama", "model_unweighted", "model_weighted"
