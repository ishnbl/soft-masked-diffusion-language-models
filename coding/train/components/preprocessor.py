...
def filter_long_inputs(features, max_length=2048):
    ...
    return features.filter(
        lambda ex: len(ex["input_ids"]) <= max_length,
...
def filter_short_responses(features, min_length=5):
    ...
    """
    return features.filter(
        lambda ex: ex["resp_lengths"] >= min_length,
        desc="filter short sequences",
        load_from_cache_file=LOAD_FROM_CACHE,
    )
...
def filter_tool_calls(features):
    ...
    return features.filter(
        lambda ex: tool_call_token not in ex["input_ids"]
        and tool_call_end_token not in ex["input_ids"],
        desc="filter 151657/151658",
        load_from_cache_file=LOAD_FROM_CACHE,
    )
...
def to_chat_batched(
    batch: Dict[str, List[Any]], dataset_args, tokenizer, max_length=2048
) -> Dict[str, List[str]]:
    ...
    if dataset_args.is_messages:
        ...
        assert (
            dataset_args.prompt_column and dataset_args.response_column
...
def load_data(args, tokenizer, test_size=0.1, seed=42):
    ...
    if args.data_path == "mix":
        d1 = to_train_val(
            load_dataset("allenai/tulu-3-sft-mixture"), test_size=test_size, seed=seed
        )
        d2 = to_train_val(
        ...
        ds = (
            load_dataset(args.data_path, args.data_subset)
            if args.data_subset
            else load_dataset(args.data_path)
        )
    ...
    raw["train"] = (
        raw["train"]
        .shuffle(seed=seed)
        .select(range(min(args.max_train_size, len(raw["train"]))))
    )
    raw["validation"] = (
        raw["validation"]
        .shuffle(seed=seed)
        .select(range(min(500, len(raw["validation"]))))
    )
    ...
    tvals = np.linspace(
        args.min_prob, args.max_prob, num=len(raw["validation"]), dtype=np.float32
    ).tolist()
    ...
    print(
        f"Train tokens: prompt {train_prompt_tokens}, response {train_resp_tokens}, total {train_prompt_tokens+train_resp_tokens}"
    )

    # set format for PyTorch
    raw["train"].set_format(
        type="torch", columns=["input_ids", "prompt_lengths", "seed"]
    )
    raw["validation"].set_format(
        type="torch", columns=["input_ids", "prompt_lengths", "t", "seed"]
    )
