...
from utils import get_batch_seed, seeded_rand, seeded_randint
...
class dLLMDataCollator(DefaultDataCollator):
    ...
    def __init__(self, tokenizer, cfg: CollatorConfig):
        ...
        self.mask_token_id = (
            tokenizer.mask_token_id if cfg.mask_token_id is None else cfg.mask_token_id
        )
        if self.mask_token_id is None:
            raise ValueError(
                "mask_token_id must be set (tokenizer.mask_token_id is None)."
            )
    ...
    def add_at_least_one_mask(
        self, labels, input_ids, prompt_lengths, mask_indices, batch_seed=None
    ):
        """
        ...
        if rows.any():
            print(
                f"Warning: {rows.sum().item()} rows have all labels as -100. Fixing by replacing one label per row."
            )
            ...
            j = (
                seeded_randint(
                    L - low, size=(1,), device=labels.device, seed=batch_seed
                )
                + low
            )
            j = int(j.item())
    ...
    def get_p_mask(self, batch, device, eps=1e-3, batch_seed=None):
        ...
        if "t" in batch:
            ...
            t = (
                ...
                * seeded_rand((), device, seed=batch_seed)
                + self.min_prob
            ).clamp_min(
                eps
            )  # Uniform[0.2,0.8]
    ...
    def prepad_input_ids(self, input_ids, pad_length, pad_token_id):
        ...
        """
        padded = torch.full(
            (pad_length,), pad_token_id, dtype=input_ids.dtype, device=input_ids.device
        )
    ...
    def get_pad_length_for_batch(self, features, batch_seed=None):
        ...
        extra_len = seeded_randint(
            50, size=(1,), device=features[0]["input_ids"].device, seed=batch_seed
        )
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for f in features:
            f["input_ids"] = self.prepad_input_ids(
                f["input_ids"], pad_length, self.tokenizer.pad_token_id
            )
        ...
        noisy_batch, labels, mask_indices, p_mask, dsigma = self.forward_process(
            batch, batch_seed
        )
        labels, noisy_batch, mask_indices = self.add_at_least_one_mask(
            labels,
            noisy_batch,
            batch["prompt_lengths"].long(),
            mask_indices,
            batch_seed,
        )
