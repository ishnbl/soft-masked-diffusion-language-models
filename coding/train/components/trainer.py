...
import torch
from components.transparency_head import get_th_kwargs, transparency_head
from torch.optim import AdamW
from transformers import Trainer
from utils import get_batch_seed_collated, seeded_rand


class dLLMTrainer(Trainer):

    def __init__(self, *args, loss_calc="model_weighted", data_collator=None, **kwargs):
            ...
            with torch.no_grad():
                ...
                if th_module is not None:
                    if getattr(th_module, "last_lambda_mean", None) is not None:
                        logs["transparency/lambda_mean"] = (
                            th_module.last_lambda_mean.item()
                        )
                    if getattr(th_module, "last_lambda_std", None) is not None:
                        logs["transparency/lambda_std"] = (
                            th_module.last_lambda_std.item()
                        )
    ...
    def save_model(self, output_dir: str | None = None, _internal_call: bool = False):
        ...
        try:
            ...
            with open(
                os.path.join(output_dir, "transparency_config.json"),
                "w",
                encoding="utf-8",
            ) as f:
            ...
            try:
                self.log({"transparency/json_saved": 0})
            except Exception:
                pass
