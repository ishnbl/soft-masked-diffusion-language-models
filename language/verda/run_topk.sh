#!/bin/bash
#
# verda/run_topk.sh — single-node multi-GPU top-k (`mixinputs_with_topk`)
# launcher for a Verda GPU VM. Hydra args mirror run_modal_multigpu_topk.py
# EXACTLY, so a VM run reproduces the Modal run. This is the top-k counterpart
# to verda/run_slerp.sh and shares no state with it (separate by design, same as
# the two Modal scripts).
#
# DDP-safety: top-k rides the SAME transparency-alg-agnostic soft-mask gate as
# slerp (algo.py MDLM_SM.nll) — the branch decision is identical on every rank
# (step-seeded Bernoulli + globally all-reduced t-band). Top-k's own forward only
# builds local, same-shaped tensors (no collectives in its data-dependent
# branch), so it adds no cross-rank divergence.
#
# USAGE (from language/, env activated):
#   BASE_CKPT=/data/mdlm_owt.ckpt bash verda/run_topk.sh
#   NUM_GPUS=4 BASE_CKPT=/data/mdlm_owt.ckpt bash verda/run_topk.sh
#   FIXED_LAMBDA=0.5 BASE_CKPT=/data/mdlm_owt.ckpt bash verda/run_topk.sh
#   SEED=2 MAX_STEPS=5000 GLOBAL_BATCH=512 PER_GPU_BATCH=16 bash verda/run_topk.sh
#
...
# device_count() drives the world size (Hydra device_count: resolver + the
# dataloader's global==batch*device_count*accum assertion), NOT trainer.devices.
# Pin CUDA_VISIBLE_DEVICES so device_count() == NUM_GPUS before any Python runs.
...
if [[ -z "$BASE_CKPT" ]]; then
  ...
  echo "            e.g. BASE_CKPT=/data/mdlm_owt.ckpt bash verda/run_topk.sh"; exit 1
fi
...
echo "[config] alg=mixinputs_with_topk  GPUs=${NUM_GPUS}  per-GPU=${PER_GPU_BATCH}  accum=${ACCUM}  global=${GLOBAL_BATCH}"
