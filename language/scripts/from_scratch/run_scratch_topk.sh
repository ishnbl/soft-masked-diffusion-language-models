#!/usr/bin/env bash
# ============================================================
# run_scratch_topk.sh — Train the top-k LERP (`mixinputs_with_topk`)
# soft-masking path from a FRESHLY INITIALIZED model (random weights), same
# architecture/data as the released mdlm_owt checkpoint (model=small,
# data=openwebtext-split).
#
# This is the from-scratch counterpart to ../../run_verda_topk.sh: identical
...
#
# This is a SEPARATE script from run_scratch_slerp.sh on purpose: the slerp
# runner is left byte-for-byte intact so that path can never be perturbed by
# a change made for top-k (same design decision as run_verda.sh /
# run_verda_topk.sh).
#
...
#   tends to get driven toward 0 (lambda -> ~0), collapsing the soft-mask
#   feedback to plain MDLM. The init below (init_scale=0.5, init_centre=-2.5)
#   is the validated fix from scripts/07_slerp_vs_topk_fixed.sh: it keeps
#   lambda near-zero while the backbone is still uniform-ish and lets it grow
#   as the backbone becomes confident over training, rather than collapsing
#   immediately. --fixed-lambda sidesteps this entirely (lambda is pinned,
#   not learned).
#
# Usage (from inside the language/ directory):
#   bash scripts/from_scratch/run_scratch_topk.sh [options]
#
...
#   bash scripts/from_scratch/run_scratch_topk.sh --num-gpus 2 --max-steps 20 \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --data)                DATA="$2";                 shift 2 ;;
    --mixinputs-k)         MIXINPUTS_K="$2";          shift 2 ;;
...
GROUP="scratch_topk_${NUM_GPUS}gpu_${MODEL}_${DATA}_seed${SEED}"
RUN_NAME="scratch-topk-${NUM_GPUS}gpu-${MODEL}-${DATA}-seed${SEED}"
RUN_OUT_DIR="${OUT_DIR}/${GROUP}"
mkdir -p "$RUN_OUT_DIR"

echo ""
echo "========================================"
echo " Top-K Soft-Masked MDLM — From Scratch"
echo "========================================"
echo " GPUs:          $NUM_GPUS"
...
# ── Launch training ───────────────────────────────────────────
# Note: NO training.finetune_path override — this intentionally starts from
