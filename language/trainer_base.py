...
class TrainerBase(L.LightningModule):
        ...
        try:
            if "loops" in checkpoint and "fit_loop" in checkpoint["loops"]:
                ...
                if "epoch_progress" in fit_loop:
                    self.fast_forward_epochs = fit_loop["epoch_progress"]["current"][
                        "completed"
                    ]
                elif "epoch_loop.epoch_progress" in fit_loop:
                    self.fast_forward_epochs = fit_loop["epoch_loop.epoch_progress"][
                        "current"
                    ]["completed"]

                # Resolve batch progress
                if (
                    "epoch_loop" in fit_loop
                    and "batch_progress" in fit_loop["epoch_loop"]
                ):
                    self.fast_forward_batches = fit_loop["epoch_loop"][
                        "batch_progress"
                    ]["current"]["completed"]
                elif "epoch_loop.batch_progress" in fit_loop:
                    self.fast_forward_batches = fit_loop["epoch_loop.batch_progress"][
                        "current"
                    ]["completed"]
                elif "batch_progress" in fit_loop:
                    self.fast_forward_batches = fit_loop["batch_progress"]["current"][
                        "completed"
                    ]
    ...
    def on_save_checkpoint(self, checkpoint):
        ...
        if (
            ...
            and hasattr(self.trainer.train_dataloader.sampler, "state_dict")
        ):
        ...
        try:
            if "loops" in checkpoint and "fit_loop" in checkpoint["loops"]:
                fit_loop = checkpoint["loops"]["fit_loop"]
                if (
                    "epoch_loop.batch_progress" in fit_loop
                    and "epoch_loop.automatic_optimization.optim_progress" in fit_loop
                ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for k, v in self.metrics.train_nlls.items():
            self.log(
                name=k, value=v.compute(), on_step=True, on_epoch=False, sync_dist=True
            )
        ...
        if p_x0 is None:
            ...
            if self.config.sampling.p_nucleus < 1:
                ...
                p_x0 = torch.zeros_like(p_x0).scatter_(
                    -1, sorted_indices, p_nucleusrobs
                )
